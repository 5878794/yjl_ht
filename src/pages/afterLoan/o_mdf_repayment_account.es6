...
let Page = {
    ...
    async run(){
        ...
        let newData = {
            bankName:data.orderRepayment?.bankName??'',
            bankNo:data.orderRepayment?.bankNo??'',
            openBank:data.orderRepayment?.openBank??''
        };
    ...
    async submitFn(state){
        ...
        await ajax.send([
            api.afterLoan_change_repayment_account(form)
        ]);
